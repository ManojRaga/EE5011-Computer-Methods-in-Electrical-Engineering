...
def polint(xarr,yarr,xx,n):
  M=len(xarr);N=len(xx)
  c=np.zeros((n+1,1));d=np.zeros((n+1,1))
  yy=np.zeros(xx.shape);dyy=np.zeros(xx.shape)
  code="""
  #include<math.h>
  int i,j,n1,m,ns,ns0;
...
# xarr=np.linspace(0,2,11)
# yarr=1/(1+xarr*xarr)
# t=np.linspace(0,np.pi,111)
# xx=np.cos(t)*1.5+1
...
# figure(1)
# semilogy(xx,np.abs(yy-y0),'ro',xx,np.abs(dyy),'k')
